...
<v-list-item-content>
  <v-row>
    ...
    <v-col class="status">
        ...
        <v-list-item-title>{{ item.fields.check_status  }}</v-list-item-title>
    </v-col>
  </v-row>
</v-list-item-content>
...
export default {
  ...
  methods:{
    async initBusinessInfo(){
      ...
      await this.axios.get('get_has_check/',
          {params:{product_status:'Listing Request',check_status:'Approved'}})
        .then((response) => {
            console.log(response);
